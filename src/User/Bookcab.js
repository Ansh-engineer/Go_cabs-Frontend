...
const BookRide = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`https://go-cabs-backend.onrender.com/car/${id}`)
      .then((resp) => {
        setCars(resp.data);
        setLoading(false);
      })
  ...
  const handleSubmit = (e) => {
    ...
    const data = {
      ...
      pickupdate, pickuptime, dropdate, droptime, fare, userName, userId,longi
    };
    console.log(data)
    ...
    if (cars.length > 0) {
      ...
      axios.post('https://go-cabs-backend.onrender.com/rides', data)
        .then((response) => {
    ...
    <div style={{ 
      ...
      <div className="max-w-md mx-auto mt-8 p-4 border rounded shadow-lg">
        ...
        <form className="book-ride-form" onSubmit={handleSubmit}>
          ...
          <a target='_blank' href='https://www.google.pl/maps/'>Redirect to Maps</a>
          <input type='text' value={longi} onChange={(e)=> setLongi(e.target.value)}/>
          <h2>PickUp Date</h2>
          <div>
            <input
              type="date"
