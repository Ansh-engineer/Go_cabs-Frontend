...
function Acabs() {
  ...
  useEffect(() => {
    async function fetchCars() {
      try {
        const response = await axios.get('https://go-cabs-backend.onrender.com/cars');
  ...
  const deletecar = (taskId) => {
    axios.delete(`https://go-cabs-backend.onrender.com/cardelete/${taskId}`);
          ...
          {filteredCars.map((car) => (
            ...
            >
              <div className="relative h-48 w-full">
                <img 
                  src={`https://go-cabs-backend.onrender.com/${car?.carImage}`} 
